...
bmap(struct inode *ip, uint bn)
{
  ...
  if(bn < NINDIRECT){
  ...
  else if((bn -= NINDIRECT) < NNINDIRECT){     
    // Two level indirected block
    ...
    if((addr = b[bn % NINDIRECT]) == 0){
      // Allocate block for saving data
  ...
  else if((bn -= NNINDIRECT) < NNNINDIRECT){    
    // Three level indirected block
...
itrunc(struct inode *ip)
{
  ...
  // Free two level indirect blocks
  ...
  // Free three level indirect blocks
